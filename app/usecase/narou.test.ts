...
describe('addNarouNovel', () => {
	test('新規に追加する小説ではなくページが同じ場合、処理をスキップする', async () => {
		...
		expect(await addNarouNovel(db, 'https://ncode.syosetu.com/n0000zz/1')).toBe('n0000zz/1は重複のためスキップしました')
	...
	test('新規に追加する小説ではなくページが異なる場合、ページを更新する', async () => {
		...
		expect(await addNarouNovel(db, 'https://ncode.syosetu.com/n0000zz/2')).toBe('n0000zzはページを2で更新しました')
		expect(spyRepositoryGetOne).toHaveBeenCalledTimes(1)
		expect(spyRepositoryGetOne).toHaveBeenCalledWith(db, 'n0000zz')
	...
	test('新規に追加する小説の場合', async () => {
		...
		expect(await addNarouNovel(db, 'https://ncode.syosetu.com/n0000zz/1')).toBe('n0000zz/1が新しく追加されました')
		expect(spyRepositoryGetOne).toHaveBeenCalledTimes(1)
		expect(spyRepositoryGetOne).toHaveBeenCalledWith(db, 'n0000zz')
