...
export function getNarouNovel(
  ...
  const ps = d1.prepare(getNarouNovelQuery).bind(args.ncode)
  return {
    then(
      onFulfilled?: (value: GetNarouNovelRow | null) => void,
      onRejected?: (reason?: any) => void,
    ) {
      ps.first<RawGetNarouNovelRow | null>()
        .then((raw: RawGetNarouNovelRow | null) =>
          raw
            ? {
                ncode: raw.ncode,
...
export function listNarouNovels(d1: D1Database): Query<D1Result<ListNarouNovelsRow>> {
  const ps = d1.prepare(listNarouNovelsQuery)
  return {
    then(
      onFulfilled?: (value: D1Result<ListNarouNovelsRow>) => void,
      onRejected?: (reason?: any) => void,
    ) {
      ps.all<RawListNarouNovelsRow>()
        .then((r: D1Result<RawListNarouNovelsRow>) => {
          return {
            ...r,
            results: r.results.map((raw: RawListNarouNovelsRow) => {
              return {
                ncode: raw.ncode,
                currentPage: raw.current_page,
              }
...
export function createNarouNovel(
  ...
  const ps = d1.prepare(createNarouNovelQuery).bind(args.ncode, args.currentPage)
  return {
    then(
      onFulfilled?: (value: CreateNarouNovelRow | null) => void,
      onRejected?: (reason?: any) => void,
    ) {
      ps.first<RawCreateNarouNovelRow | null>()
        .then((raw: RawCreateNarouNovelRow | null) =>
          raw
            ? {
                ncode: raw.ncode,
                currentPage: raw.current_page,
...
export function updateNarouNovel(d1: D1Database, args: UpdateNarouNovelParams): Query<D1Result> {
  const ps = d1.prepare(updateNarouNovelQuery).bind(args.currentPage, args.ncode)
  return {
...
export function deleteNarouNovel(d1: D1Database, args: DeleteNarouNovelParams): Query<D1Result> {
  const ps = d1.prepare(deleteNarouNovelQuery).bind(args.ncode)
  return {
    then(onFulfilled?: (value: D1Result) => void, onRejected?: (reason?: any) => void) {
      ps.run().then(onFulfilled).catch(onRejected)
    },
